...
function CitySelect({
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch('/data/kota-indonesia.json', {
          cache: 'force-cache',
        });
...
export default function RegistrationForm({
    ...
    (async () => {
      try {
        const res = await fetch(
          `${API_BASE}/dashboard/ticket-transactions/quote?event_ticket_id=${ticket.id}&quantity=${qty}`,
          {
            method: 'GET',
            headers: {
              'x-api-key': API_KEY,
        ...
        if (json?.success && json.data) {
          ...
          setAdminFee(json.data.admin_fee ?? 0);
  ...
  const handlePaidOrPending = useCallback(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      (window as any).snap.pay(snapToken, {
        onSuccess: async () => {
          await handlePaidOrPending(orderId);
        },
        onPending: async () => {
          await handlePaidOrPending(orderId);
        },
