...
import { referralService } from '@/lib/data-service';
...
export default function RegistrationForm({ event, ticket }: { event: Event; ticket: EventTicket }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoadingRef(true);
        const list = await referralService.list();
        if (!alive) return;
  ...
  const openTicketTabs = (uuids: string[]) => {
    ...
    uuids.forEach((u, idx) => {
      if (idx > 0) window.open(`/ticket/${u}`, '_blank');
    });
              ...
              <div className="px-5 py-5 md:px-6 md:py-6">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    ...
                    {loadingRef && (
                      <div className="mt-1 text-xs text-slate-500">Memuat daftar referral…</div>
                    )}
                  </div>
                </div>
              </div>
