...
const fmtDateRange = (startISO?: string, endISO?: string) => {
  ...
  if (sameDay) return `${fmtDate(startISO)} pukul ${fmtTime(startISO)} - ${fmtTime(endISO)}`;
  return `${fmtDate(startISO)} ${fmtTime(startISO)} - ${fmtDate(endISO)} ${fmtTime(endISO)}`;
};

function CitySelect({
  value, onChange, placeholder = 'Cari & pilih kota (cth: Bandung)', required = true,
  ...
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') { e.preventDefault(); setHighlight((h) => Math.min(h + 1, filtered.length - 1)); }
      ...
      if (pick) { onChange(pick); setOpen(false); }
    ...
    <div ref={wrapRef} className="relative z-50">
      ...
      {open && (
        <div className="absolute z-[9999] mt-1 w-full rounded-xl border border-slate-200 bg-white shadow-xl">
          <div className="border-b bg-slate-50 px-3 py-2 text-xs text-slate-500">Ketik untuk mencari kota.</div>
              ...
              filtered.map((c, i) => {
                ...
                return (
                  <button
                    type="button"
                    key={c}
                    className={`flex w-full items-center justify-between px-3 py-2 text-left text-sm ${isActive ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50'}`}
...
type StoreResp = { success: boolean; message?: string; data?: { order_id: string; snap_token: string } };
...
export default function RegistrationForm({ event, ticket }: { event: Event; ticket: EventTicket }) {
  ...
  const updateTicketHolder = (i: number, field: 'name'|'email'|'phone', value: string) => setTicketHolders((v) => v.map((h, idx) => (idx === i ? { ...h, [field]: value } : h)));
  ...
  const resetSnapArtifacts = useCallback(() => {
    ...
    ['overflow','paddingRight','marginRight','position','height','width'].forEach((k) => { // @ts-ignore
      html.style[k] = ''; // @ts-ignore
      body.style[k] = '';
    });
    ['snap-body','snap-open','modal-open','swal2-shown'].forEach((cls) => { html.classList.remove(cls); body.classList.remove(cls); });
  }, []);
  ...
  async function confirmOnServer(orderId: string): Promise<string[] | null> {
    ...
    return resp?.success ? ((resp?.uuids as string[]) || null) : null;
  }

  const openTicketTabs = (uuids: string[]) => {
    if (!uuids?.length) return;
    uuids.forEach((u, idx) => { if (idx > 0) window.open(`/ticket/${u}`, '_blank'); });
    router.push(`/ticket/${uuids[0]}`);
  };
    ...
    try {
      ...
      const created: StoreResp = await fetch(`${API_BASE}/dashboard/ticket-transactions`, {
        ...
        body: JSON.stringify({
          ...
          ticket_holders: ticketHolders.map((h) => ({ name: h.name, email: h.email, phone: h.phone })),
      ...
      if (!orderId || !snapToken) throw new Error('Gagal membuat transaksi (order_id/snap_token kosong).');

      await loadSnap();
      (window as any).snap.pay(snapToken, {
        onSuccess: async () => {
          const uuids = await confirmOnServer(orderId);
          setRedirecting(true);
          resetSnapArtifacts();
          if (uuids?.length) openTicketTabs(uuids);
        },
        onPending: async () => {
          const uuids = await confirmOnServer(orderId);
          setRedirecting(true);
        ...
        onError: () => { setErrorMsg('Pembayaran gagal. Coba lagi.'); resetSnapArtifacts(); },
  ...
  return (
    <div className="min-h-screen bg-white antialiased">
      <div className="border-b bg-white">
        <Container className="py-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <Link href={`/events/${event.slug}`} className="inline-flex items-center text-sm font-medium text-blue-600 hover:text-blue-700 hover:underline">
              <svg className="mr-2 h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M12.293 15.707a1 1 0 01-1.414 0l-5-5a1 1 0 010-1.414l5-5a1 1 0 111.414 1.414L8.414 10l3.879 3.879a1 1 0 010 1.414z" clipRule="evenodd"/></svg>
              Kembali ke Acara
            </Link>
          </div>

          <div className="mt-3">
            <h1 className="text-2xl font-bold md:text-3xl">Daftar untuk <span className="text-blue-700">{event.title}</span></h1>
            <p className="mt-1 text-sm text-slate-500">Lengkapi data di bawah untuk menyelesaikan pendaftaran kamu.</p>
          </div>
        </Container>
      </div>
          ...
          <form id="regForm" onSubmit={handleSubmit} className="space-y-6">
            <Card className="overflow-visible border-slate-200 p-0 shadow-sm">
              <div className="border-b bg-slate-50 px-5 py-4 md:px-6">
                ...
                <p className="mt-1 text-xs text-slate-500">Data ini digunakan untuk invoice & notifikasi.</p>
              </div>
              <div className="px-5 py-5 md:px-6 md:py-6">
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  <div><label className="mb-1 block text-sm font-medium">Nama Lengkap *</label>
                    <Input className="bg-white" placeholder="Nama sesuai KTP" value={buyerInfo.name} onChange={(e) => updateBuyerInfo('name', e.target.value)} autoComplete="off" required />
                  </div>
                  <div><label className="mb-1 block text-sm font-medium">Alamat Email *</label>
                    <Input className="bg-white" type="email" placeholder="nama@email.com" value={buyerInfo.email} onChange={(e) => updateBuyerInfo('email', e.target.value)} autoComplete="off" required />
                  </div>
                  <div><label className="mb-1 block text-sm font-medium">Nomor Telepon *</label>
                    <Input className="bg-white" inputMode="tel" placeholder="08xxxxxxxxxx" value={buyerInfo.phone} onChange={(e) => updateBuyerInfo('phone', e.target.value)} autoComplete="off" required />
                  </div>
                  <div><label className="mb-1 block text-sm font-medium">Asal Kota *</label>
                    <CitySelect value={buyerInfo.city} onChange={(v) => updateBuyerInfo('city', v)} />
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card className="overflow-visible border-slate-200 p-0 shadow-sm">
              <div className="flex items-center justify-between border-b bg-slate-50 px-5 py-4 md:px-6">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">Nama akan muncul pada tiket & QR check-in.</p>
                </div>
                <Button type="button" onClick={addTicketHolder} className="h-9 rounded-full bg-white px-4 text-xs font-semibold text-blue-700 ring-1 ring-blue-200 hover:bg-blue-50">+ Tambah Pemegang</Button>
              </div>

              <div className="px-5 py-5 md:px-6 md:py-6">
                <div className="space-y-4">
                  {ticketHolders.map((h, i) => (
                    <div key={i} className="rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_0_rgba(15,23,42,0.03)]">
                      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
                        <p className="max-w-[70%] truncate text-sm font-medium sm:max-w-none">Pemegang Tiket {i + 1}</p>
                        {ticketHolders.length > 1 && (
                          <button type="button" onClick={() => removeTicketHolder(i)} className="rounded-full px-3 py-1 text-xs font-semibold text-rose-600 hover:bg-rose-50">Hapus</button>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                        <div>
                          ...
                          <Input className="bg-white" placeholder="Nama lengkap" value={h.name} onChange={(e) => updateTicketHolder(i, 'name', e.target.value)} autoComplete="off" required />
                        </div>
                        <div>
                          <label className="mb-1 block text-sm font-medium">Alamat Email *</label>
                          <Input className="bg-white" type="email" placeholder="email@contoh.com" value={h.email} onChange={(e) => updateTicketHolder(i, 'email', e.target.value)} autoComplete="off" required />
                        </div>
                        <div>
                          <label className="mb-1 block text-sm font-medium">Nomor Telepon *</label>
                          <Input className="bg-white" inputMode="tel" placeholder="08xxxxxxxxxx" value={h.phone} onChange={(e) => updateTicketHolder(i, 'phone', e.target.value)} autoComplete="off" required />
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {errorMsg ? <div className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{errorMsg}</div> : null}
          </form>

          <aside className="lg:sticky lg:top-6">
            <Card className="overflow-hidden border-slate-200 shadow-sm">
              <div className="border-b bg-slate-50 px-5 py-4 md:px-6"><h3 className="text-base font-semibold">Ringkasan Pembayaran</h3></div>
                ...
                <div>
                  ...
                  <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                    ...
                    <p className="flex items-start">
                      <svg className="mr-2 mt-0.5 h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1z" clipRule="evenodd"/></svg>
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white md:hidden" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <div className="mx-auto flex w-full max-w-3xl items-center justify-between gap-3 px-4 py-3">
          <div className="min-w-0">
            <div className="text-xs text-slate-500">Total</div>
            <div className="truncate text-lg font-bold">{rupiah(total)}</div>
          </div>
          <Button form="regForm" type="submit" className="h-11 flex-1 rounded-full bg-blue-700 font-semibold text-white shadow-md hover:bg-blue-800 disabled:cursor-not-allowed disabled:bg-gray-300" disabled={!canSubmit}>
            {submitting ? 'Memproses…' : 'Lanjutkan Pembayaran'}
          </Button>
        </div>
      </div>
